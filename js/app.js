...
;(function (root, factory) { const api = factory(); if (typeof module === 'object' && module.exports) module.exports = api; if (root) { root.AstroEducation = root.AstroEducation || {}; root.AstroEducation.App = api; } })(typeof globalThis !== 'undefined' ? globalThis : this, function () {
  ...
  function updateSceneButtons(d, selected) { ['heliocentric', 'geocentric'].forEach(function (id) { const node = d.getElementById(id + 'SceneBtn'); if (node && node.setAttribute) { const active = id === selected; node.setAttribute('aria-pressed', String(active)); node.setAttribute('aria-selected', String(active)); } const panel = d.getElementById(id + 'Viewport'); if (panel && panel.setAttribute) panel.setAttribute('aria-hidden', String(id !== selected)); }); }
  function solarTermNames(education) { const terms = education.Astronomy && education.Astronomy.SOLAR_TERMS; return Array.isArray(terms) ? terms.map(function (term) { return typeof term === 'string' ? term : term.name; }) : []; }
