...
;(function (root, factory) { const api = factory(); if (typeof module === 'object' && module.exports) module.exports = api; if (root) { root.AstroEducation = root.AstroEducation || {}; root.AstroEducation.App = api; } })(typeof globalThis !== 'undefined' ? globalThis : this, function () {
  ...
  function start(options) {
    ...
    let host = null, scenes = null, currentState = null, requestId = null, stopped = false, hiddenWasPlaying = false, previousFrameTime = null, redrawRequested = false;
    ...
    function applyControllerState(state) { currentState = education.WorldState.create({ instantUtc: state.instantUtc, timeZone: state.timeZone, location: state.location }); updateHud(documentRef, currentState); if (host) host.renderFrame(currentState); requestRedraw(); }
    ...
    function bindControls() { Object.keys(scenes).forEach(function (id) { const controls = scenes[id].controls; listen(controls, 'change', requestRedraw); listen(controls, 'start', requestRedraw); listen(controls, 'end', requestRedraw); }); }
    function bindResponsive() { const queries = [['(max-width: 600px)', 'mobile'], ['(max-width: 1024px)', 'tablet']]; queries.forEach(function (pair) { const query = windowRef.matchMedia && windowRef.matchMedia(pair[0]); const change = function () { host.setLayoutMode(modeForWidth(education, windowRef.innerWidth)); requestRedraw(); }; listen(query, 'change', change); }); }
    function bindButtons() { listen(documentRef.getElementById('resetBtn'), 'click', function () { Object.keys(scenes).forEach(function (id) { if (scenes[id].resetView) scenes[id].resetView(); }); requestRedraw(); }); listen(documentRef.getElementById('snapBtn'), 'click', function () { exportSnapshot(documentRef, canvas, host, currentState); }); ['heliocentric', 'geocentric'].forEach(function (id) { const button = documentRef.getElementById(id + 'SceneBtn'); listen(button, 'click', function () { host.setSelectedScene(id); ['heliocentric', 'geocentric'].forEach(function (other) { const node = documentRef.getElementById(other + 'SceneBtn'); if (node && node.setAttribute) node.setAttribute('aria-pressed', String(other === id)); }); requestRedraw(); }); }); }
    ...
    function onVisibility() { if (documentRef.hidden) { hiddenWasPlaying = timeController.getState().playing; if (hiddenWasPlaying) timeController.pause(); previousFrameTime = null; if (requestId && caf) caf(requestId); requestId = null; } else { previousFrameTime = null; if (hiddenWasPlaying && !reducedMotion) timeController.play(); hiddenWasPlaying = false; requestRedraw(); } }
    function stop() { if (stopped) return; stopped = true; if (requestId && caf) caf(requestId); unsubscribe(); listeners.forEach(function (l) { l[0].removeEventListener(l[1], l[2]); }); if (host) host.dispose(); }
  }
  function modeForWidth(education, width) { return education.SceneHost.layoutModeForWidth ? education.SceneHost.layoutModeForWidth(width || 0) : 'desktop'; }
