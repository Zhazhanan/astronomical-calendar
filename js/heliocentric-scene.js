;(function (root, factory) {
  ...
  function create(config) {
    ...
    const texture = new THREE.CanvasTexture({}); resources.push(texture); const glow = new THREE.Sprite(new THREE.SpriteMaterial({ map: texture, color: 0xffb342, transparent: true, opacity: 0.42, blending: THREE.AdditiveBlending })); resources.push(glow.material); if (glow.scale && glow.scale.set) glow.scale.set(13, 13, 1); root.add(glow);
    ...
    const moonMaterial = new THREE.ShaderMaterial({ uniforms: { lightDirection: { value: new THREE.Vector3(1, 0, 0) }, illumination: { value: 0 } }, vertexShader: 'varying vec3 vNormal; void main(){vNormal=normalize(normalMatrix*normal);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}', fragmentShader: 'uniform vec3 lightDirection; varying vec3 vNormal; void main(){float day=max(dot(normalize(vNormal),normalize(lightDirection)),0.0);gl_FragColor=vec4(vec3(0.78)*mix(0.08,1.0,day),1.0);}' }); resources.push(moonMaterial);
    ...
    let lastState = null, lastOrbit = null, disposed = false;
    function update(s) {
      ...
      if (orbitChanged(lastOrbit, s.moon)) { writeMoonOrbit(moonOrbitGeometry, s.moon, nodeA, nodeD, peri, apo); setLineEndpoints(nodeLine.geometry, nodeA, nodeD); ascendingNode.position.copy(nodeA); descendingNode.position.copy(nodeD); perigee.position.copy(peri); apogee.position.copy(apo); updateOrbitLabels(labels, s.moon, nodeA, nodeD, peri, apo); lastOrbit = orbitSnapshot(s.moon); }
    }
    ...
    scene.userData = { earthGroup, earthAxisGroup, earthSpinGroup, observerMarker, moon, moonOrbit, labels };
  ...
  function orbitSnapshot(m) { return { n: m.ascendingNodeLongitudeDeg, i: m.orbit.inclinationDeg, a: m.argumentOfPerigeeDeg, e: m.orbit.eccentricity }; }
  function orbitChanged(last, m) { return !last || Math.abs(last.n - m.ascendingNodeLongitudeDeg) >= ORBIT_CHANGE_THRESHOLD_DEG || Math.abs(last.i - m.orbit.inclinationDeg) >= ORBIT_CHANGE_THRESHOLD_DEG || Math.abs(last.a - m.argumentOfPerigeeDeg) >= ORBIT_CHANGE_THRESHOLD_DEG || Math.abs(last.e - m.orbit.eccentricity) >= 1e-5; }
  ...
  function writeOrbitPoint(values, offset, anomaly, moon) { const p = { set: function (x, y, z) { this.x = x; this.y = y; this.z = z; } }; orbitPoint(p, anomaly, moon); values[offset] = p.x; values[offset + 1] = p.y; values[offset + 2] = p.z; }
  ...
  return Object.freeze({ DISPLAY, toVisualModel, create });
});
