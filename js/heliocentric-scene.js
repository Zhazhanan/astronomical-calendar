;(function (root, factory) {
  ...
  function create(config) {
    ...
    const scene = new THREE.Scene(), camera = new THREE.PerspectiveCamera(45, 1, 0.1, 200), resources = [], labels = createLabels(THREE, options.labelLayer), root = new THREE.Group();
    ...
    labels.slice(1, 5).forEach(function(label) { label.anchorParent = earthGroup; });
    ...
    const up = new THREE.Vector3(0, 1, 0), axis = new THREE.Vector3(), sunDirection = new THREE.Vector3(), moonVector = new THREE.Vector3(), observerVector = new THREE.Vector3(), nodeA = new THREE.Vector3(), nodeD = new THREE.Vector3(), peri = new THREE.Vector3(), apo = new THREE.Vector3(), projected = new THREE.Vector3();
    ...
    function updateLabelPositions() {
      LabelLayout.position(labels, camera, projected, options);
    }
    ...
    return Object.freeze({ scene, camera, controls, update, render, updateLabelPositions, resetView, setLabelsVisible: function (visible) { labels.forEach(function (l) { l.style.display = visible ? '' : 'none'; }); }, dispose });
  }
  function createLabels(THREE, layer) { return ['太阳', '升交点', '降交点', '近地点', '远地点', LEGEND].map(function (text, index) { const e = typeof document !== 'undefined' && document.createElement ? document.createElement('span') : { style: {} }; e.textContent = text; e.className = 'scene-label'; e.style = e.style || {}; e.style.position = 'absolute'; e.style.pointerEvents = 'none'; e.style.visibility = 'hidden'; e.anchor = new THREE.Vector3(); e.offsetY = index === 0 ? -32 : 0; e.hudSlot = index === 5 ? { x: 0.04, y: 0.90 } : null; if (layer && layer.appendChild) layer.appendChild(e); return e; }); }
  ...
  function updateOrbitLabels(labels, moon, a, d, p, o) { [['升交点', moon.ascendingNodeLongitudeDeg, a], ['降交点', moon.descendingNodeLongitudeDeg, d], ['近地点', moon.perigeeDistanceKm, p], ['远地点', moon.apogeeDistanceKm, o]].forEach(function (entry, i) { const l = labels[i + 1], point = entry[2]; l.textContent = `${entry[0]} ${Math.round(entry[1] || 0)}${i < 2 ? '°' : ' km'}`; l.anchor.copy(point); Object.assign(l.dataset || {}, { x: String(point.x), y: String(point.y), z: String(point.z) }); }); }
