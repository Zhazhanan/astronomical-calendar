;(function (root, factory) {
  ...
  function rectFor(element) { return element && element.getBoundingClientRect ? element.getBoundingClientRect() : { left: 0, top: 0, width: 1, height: 1 }; }
  function createLabel(THREE, layer, text, records, hudSlot) { const element = typeof document !== 'undefined' && document.createElement ? document.createElement('span') : { style: {}, textContent: '', className: '' }; element.className = 'scene-label'; element.textContent = text; element.style.position = 'absolute'; element.style.pointerEvents = 'none'; const record = { element: element, anchor: hudSlot ? null : new THREE.Vector3(), hudSlot: hudSlot || null }; element.anchor = record.anchor; element.hudSlot = record.hudSlot; if (layer && layer.appendChild) layer.appendChild(element); records.push(record); return record; }
  function buildLabels(THREE, layer, names, records) { const labels = { current: createLabel(THREE, layer, '当前节气：—', records, { x: 0.04, y: 0.06 }), next: createLabel(THREE, layer, '下一节气：—', records, { x: 0.04, y: 0.13 }), latitude: createLabel(THREE, layer, '月球黄纬：—', records), node: createLabel(THREE, layer, '距交点：—', records), phase: createLabel(THREE, layer, '月相：—', records), eclipse: createLabel(THREE, layer, '食季：—', records, { x: 0.04, y: 0.20 }), lambda: createLabel(THREE, layer, 'λ☉ = —', records), moon: null, projection: null }; labels.moon = labels.latitude; labels.projection = labels.node; for (let index = 0; index < 24; index++) { const degrees = index * 15, name = names[index] || String(degrees) + '°', cardinal = degrees === 0 ? '春分' : degrees === 90 ? '夏至' : degrees === 180 ? '秋分' : degrees === 270 ? '冬至' : null, record = createLabel(THREE, layer, cardinal ? degrees + '° · ' + cardinal : name, records); setVectorFromLonLat(record.anchor, degrees, 0, DISPLAY.sphereRadius + 1.7); } setVectorFromLonLat(labels.lambda.anchor, 0, 0, DISPLAY.sphereRadius * 0.62); return labels; }
