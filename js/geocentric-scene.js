;(function (root, factory) {
  const api = factory(typeof module === 'object' && module.exports ? require('./scene-label-layout.js') : root.AstroEducation.SceneLabelLayout);
  if (typeof module === 'object' && module.exports) module.exports = api;
  if (root) { root.AstroEducation = root.AstroEducation || {}; root.AstroEducation.GeocentricScene = api; }
})(typeof globalThis !== 'undefined' ? globalThis : this, function (LabelLayout) {
  'use strict';
  ...
  function arcNeedsRewrite(next, previous) { return !finite(previous) || Math.abs(normalDegrees(next) - normalDegrees(previous)) > 180 || Math.abs(normalDegrees(next) - normalDegrees(previous)) + 1e-9 >= DISPLAY.longitudeThresholdDeg; }
  ...
  function create(options) {
    ...
    const equatorMaterial = new THREE.LineDashedMaterial({ color: 0x22d3ee, dashSize: 1, gapSize: 0.65 }); resources.push(equatorGeometry, equatorMaterial); const equator = new THREE.Line(equatorGeometry, equatorMaterial); if (equator.computeLineDistances) equator.computeLineDistances();
    ...
    function update(state) {
      ...
      if (arcNeedsRewrite(solarLongitude, lastSolarLongitude)) { writeLongitudeArc(solarArc.geometry, solarLongitude, 0, DISPLAY.sphereRadius * 0.55); lastSolarLongitude = solarLongitude; }
      ...
      labels.moon.anchor.copy(moonPosition); labels.projection.anchor.copy(projectionPosition); labels.node.anchor.copy(projectionPosition); labels.node.anchor.y += 1.5; labels.phase.anchor.copy(moonPosition); labels.phase.anchor.y += 1.5;
      if (arcNeedsRewrite(state.moon.longitudeDeg, lastMoonLongitude) || Math.abs(state.moon.latitudeDeg - lastMoonLatitude) + 1e-9 >= DISPLAY.longitudeThresholdDeg) { writeLatitudeArc(latitudeArc.geometry, state.moon.longitudeDeg, state.moon.latitudeDeg, DISPLAY.sphereRadius * 0.84); lastMoonLongitude = normalDegrees(state.moon.longitudeDeg); lastMoonLatitude = state.moon.latitudeDeg; }
      updateDynamicLabels(labels, state, solarLongitude);
    }
    function updateLabelPositions() { LabelLayout.position(labelRecords, camera, projected, config); }
  ...
  function createLabel(THREE, layer, text, records, hudSlot) { const element = typeof document !== 'undefined' && document.createElement ? document.createElement('span') : { style: {}, textContent: '', className: '' }; element.className = 'scene-label'; element.textContent = text; element.style.position = 'absolute'; element.style.pointerEvents = 'none'; element.style.visibility = 'hidden'; const record = { element: element, anchor: hudSlot ? null : new THREE.Vector3(), hudSlot: hudSlot || null }; element.anchor = record.anchor; element.hudSlot = record.hudSlot; if (layer && layer.appendChild) layer.appendChild(element); records.push(record); return record; }
  function buildLabels(THREE, layer, names, records) { const labels = { current: createLabel(THREE, layer, '当前节气：—', records, { x: 0.04, y: 0.70 }), next: createLabel(THREE, layer, '下一节气：—', records, { x: 0.04, y: 0.78 }), latitude: createLabel(THREE, layer, '月球黄纬：—', records), node: createLabel(THREE, layer, '距交点：—', records), phase: createLabel(THREE, layer, '月相：—', records), eclipse: createLabel(THREE, layer, '食季：—', records, { x: 0.04, y: 0.86 }), lambda: createLabel(THREE, layer, 'λ☉ = —', records), moon: null, projection: null }; labels.moon = labels.latitude; labels.projection = labels.node; for (let index = 0; index < 24; index++) { const degrees = index * 15, name = names[index] || String(degrees) + '°', cardinal = degrees === 0 ? '春分' : degrees === 90 ? '夏至' : degrees === 180 ? '秋分' : degrees === 270 ? '冬至' : null, record = createLabel(THREE, layer, cardinal ? degrees + '° · ' + cardinal : name, records); record.keepAnchor = true; record.priority = cardinal ? 1 : 0; setVectorFromLonLat(record.anchor, degrees, 0, DISPLAY.sphereRadius + 1.7); } setVectorFromLonLat(labels.lambda.anchor, 0, 0, DISPLAY.sphereRadius * 0.62); return labels; }
